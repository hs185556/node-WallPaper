(function (window) {
	...
	load();
	//1.判断是否在瀑布流页面
	var isInPicWater = true;
	var tabs = document.getElementById('tab_header').querySelectorAll('li');
	for(var i=0;i<tabs.length;i++){
		tabs[i].onclick = ()=>{
			if(i!=0)isInPicWater=false;
			else isInPicWater=true;
		}
	}
    // 2. 调用选项卡
    tab();
    ...
    window.onscroll = function () {
		...
		//2返回顶部的显示隐藏
		var rockToTop = document.getElementById('toTop');
		scroll().top > 500 ? show(rockToTop) : hide(rockToTop); // 如果大于600就显示 否则隐藏
    };
    // 5. 广告轮播
    bannerAutoPlay();
    //6.返回顶部
...
function load(){
	...
	var oMain = document.querySelector('.page');
	setTimeout(function(){
		startMove(oLoad,{opacity:0},()=>{
			oLoad.style.display = 'none';
			oMain.style.display = 'block';
			document.body.style.background = 'inherit';
			document.body.style.color = 'inherit';
			startMove(oMain,{opacity:100});
 ...
 */
function toTop() {
	var oBtn = document.querySelector("#toTop img");
	var timer = null;
